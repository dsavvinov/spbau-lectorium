...
Отмена возможна, только если задача ещё не выполнена. При этом, если она выполняется уже, то она отменится только в случае вызова `cancel(true)`.
...
* Результаты упорядоены, но не вызовы

для паралельного исполнени стрима, либо берем `Collection.parallelStream()`, либо от последовательного стрима можно взять `.parallel()`. 
...
`R collect(collect(Supplier<R> supplier, BiConsumer<R,? super T> accumulator, BiConsumer<R,R> combiner))`
